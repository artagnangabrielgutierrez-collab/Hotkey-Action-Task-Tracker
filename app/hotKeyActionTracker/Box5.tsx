import { useDashboardInfo, useActiveTab, DashboardInfo } from "@/store/useGlobalStore";

interface Box5Props {
  dashboardInfoItems: DashboardInfo
}

function TaskCard({ dashboardInfoItems }: Box5Props) {
  const { name, hotkey, totalCompletion, completionAnimation } =
    dashboardInfoItems;
  return (
    <div className="flex flex-col gap-1 p-2 rounded-lg border border-[#1d4ed8]/50 bg-[#00040f]/50">
  ...
  return (
    <div className="group bg-[#00040f] border border-[#1d4ed8] rounded-lg text-[#bfdbfe] h-full">
      ...
      >
        ...
        <div className="mt-2 flex-1 w-full overflow-auto">
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {dashboardInfo
              .filter((e) => e.id != activeTab)
              .map((e, i) => (
                <TaskCard key={e.id} dashboardInfoItems={e} />
              ))}
          </div>
        </div>
      </div>
